...
import numpy as np
import matplotlib
matplotlib.use('Agg')  # headless safe; remove if running locally with display
...
def run_simulation(
    ...
    print(f"\n{'='*62}")
    print(f"  FBR Model v3  |  mode={mode}  |  tissue={tissue_context}")
    print(f"  E={E_kPa} kPa | Ra={Ra_um} µm | coating={coating} | drug={drug}")
    print(f"{'='*62}\n")
    ...
    if drug != "none" and drug in DRUG_OPTIONS and DRUG_OPTIONS[drug]:
        ...
        print(f"[Drug] Profile loaded: {drug_fn.__name__}")
    ...
    surface = SurfaceInputs(
        ...
        coating=coating,
        drug_release_profile=drug_fn,
    )
    print(f"[Surface] DAMP={surface.damp_signal:.3f} | "
          f"Protein adsorption={surface.protein_adsorption_score:.3f} | "
          f"Coating suppression={surface.coating_suppression:.0%}")
    ...
    nfkb_ts = NFkBModule(surface, ca_ts).simulate(t_minutes)
    ...
    classifier = PhenotypeClassifier(
        yap_nuclear=yap_ts,
        nfkb_active=nfkb_ts,
        stat6_active=stat6_ts,
        t_minutes=t_minutes,
        mcsf_bias=surface.mcsf_bias,
        mode=mode,
    ...
    if save_outputs:
        ...
        results.save_csv(out_dir / f"results_{mode}_{coating}_{drug}.csv")
 
    if plot:
        plot_results(results,
                     save_path=f"outputs_data/plot_{mode}_{coating}_{drug}.png")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="FBR Mechanotransduction Model v3")
    ...
    run_simulation(
        E_kPa=args.stiffness,
        Ra_um=args.roughness,
