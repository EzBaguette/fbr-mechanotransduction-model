...
K_IKK=0.04; K_IKK_OFF=0.02; K_NUC=0.06; K_EXP=0.01
K_IKBA=0.08; K_IKBA_SYNTH=0.08; K_IKBA_DEG=0.04; K_IKBA_BASE=0.003
T_HALF_ACUTE=1.5*24*60; CHRONIC_FLOOR=0.04
IKK_A_0=0.0; NFKB_N_0=0.05; IKBA_0=0.80
 
 
def damp_drive(t_min, damp_score, drug_fn=None):
    decay = np.exp(-0.693 * t_min / T_HALF_ACUTE)
    drive = float(np.clip(damp_score*decay + damp_score*CHRONIC_FLOOR, 0, 1))
    if drug_fn is not None:
        drive = drive * (1.0 - float(np.clip(drug_fn(t_min), 0, 1)))
    return drive
 
 
class NFkBModule:
    def __init__(self, surface: SurfaceInputs, ca_timeseries: np.ndarray,
                 sasp_amplification: np.ndarray = None):
        self.surface   = surface
        self.ca        = ca_timeseries
        self.damp_score= surface.damp_signal
        self.drug_fn   = surface.drug_release_profile
        self.sasp_amp  = sasp_amplification  # NEW v5
 
    def _odes(self, t, y, t_arr, ca_arr, sasp_arr):
        ikk, nfkb_n, ikba = [max(v, 0.0) for v in y]
        ca     = float(np.interp(t, t_arr, ca_arr))
        drive  = damp_drive(t, self.damp_score, self.drug_fn)
        ca_amp = 1.0 + 0.15 * min(ca / 0.5, 1.0)
        if sasp_arr is not None:
            drive = float(np.clip(drive + float(np.interp(t, t_arr, sasp_arr)), 0, 1))
    ...
    def simulate(self, t_minutes: np.ndarray) -> np.ndarray:
        sol = solve_ivp(self._odes, (t_minutes[0], t_minutes[-1]),
                        [IKK_A_0, NFKB_N_0, IKBA_0], t_eval=t_minutes,
                        args=(t_minutes, self.ca, self.sasp_amp),
